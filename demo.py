...
bwa_mem_template = "bwa mem -t 4 -R '@RG\tID:SRR1770413\tPL:illumina\tSM:E.coli_K12' demo/genome/E.coli_K12_MG1655.fa {} {} | samtools view -Sb - > demo/output/{}.bwa_mem.bam"
bwa_mem_cmd = bwa_mem_template.format(fq1, fq2, ID)
# append(ID, procedure, cmd, target=None, log), check pipeline.py for detail
pipeline.append(ID, "bwa_mem", bwa_mem_cmd, log = "demo/output/bwa_mem.log")


reorder_template = "samtools sort -@ 4 -m 4G -O bam \
...
print_reads_template = "java -Xmx16g -Djava.io.tmpdir=/tmp \
...
gatk_vcf_template = "/usr/bin/java -Xmx16g -Djava.io.tmpdir=/tmp \
